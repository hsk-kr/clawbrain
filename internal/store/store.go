...
// createTextIndex creates a full-text index on the "text" payload field.
func (s *Store) createTextIndex(ctx context.Context, collection string) error {
	wait := true
...
// AddText stores a text memory in the given collection.
// It uses Qdrant's full-text index for search — no embedding vectors needed.
// The text is stored in the "text" payload field. Additional metadata from
// extraPayload is merged into the payload.
// If id is empty, a UUID is generated.
func (s *Store) AddText(ctx context.Context, collection string, id string, text string, extraPayload map[string]any) (string, error) {
	if err := s.ensureTextCollection(ctx, collection); err != nil {
		return "", err
	}
...
// TextResult represents a single text retrieval result.
type TextResult struct {
	ID      string         `json:"id"`
	Payload map[string]any `json:"payload"`
}

// RetrieveText searches for memories matching the query text using Qdrant's
// full-text index. It updates last_accessed on all returned points.
func (s *Store) RetrieveText(ctx context.Context, collection string, query string, limit uint64) ([]TextResult, error) {
	...
	out := make([]TextResult, 0, len(points))
